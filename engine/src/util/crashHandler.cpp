...
namespace Diamond {
	...
	void CrashHandler::writeFullDump(EXCEPTION_POINTERS* exceptionPtrs, const std::string& name) {
	...
	LONG WINAPI CrashHandler::exceptionFilter(struct _EXCEPTION_POINTERS* exceptionInfo) {
		std::string name = getTimeAndDateAsString();
		...
		if (MessageBox(NULL, crashMessage.c_str(), L"Game Crash!", MB_YESNO) == IDYES) {
			...
			std::ofstream dst(formatChar("crashes/%s.log", name.c_str()), std::ios::binary);
			dst << src.rdbuf();
		}
		exit(EXIT_FAILURE);
